...
class LpSpider(scrapy.Spider):
    ...
    def start_requests(self):
        base_url = [
            'https://www.liepin.com/zhaopin/?init=-1&headckid=10309442b4fc4250&fromSearchBtn=2&pubTime=1&dqs=&ckid=48be21f35417fec6&degradeFlag=0&siTag=1B2M2Y8AsgTpgAmY7PhCfg%7EV6MwPcZ2ne9zYObRj7X8Rg&d_sfrom=search_fp_nvbar&d_ckId=5bbcb19abb610f06d82310563dd69691&d_curPage=1000&d_pageSize=40&d_headId=7cd3a89b67e7261f0646fe4114c38c34', ]
        ...
        if result_count == "10000+":  # 如果结果很多，就按照不同专业爬取
            ...
            for url in all_industry_url_list:
                yield Request(
                    ...
                    headers=self.headers,
                    cookies=self.cookies_dict,
                    callback=self.parse_by_city,
                )
        else:
            for page in range(0, 100):
                yield Request(
                    url=f"{response.url}&curPage={page}",
                    headers=self.headers,
                    cookies=self.cookies_dict,
                    callback=self.parse,
                    # meta={"proxy": "http://" + random.choice(IPPOOL)}
    ...
    def parse_by_city(self, response):
        ...
        if result_count == "10000+":
            city_url = response.xpath("//dd[@data-param='city']//a")[1:13]
            all_city_url = []
            for url in city_url:
                this_url = url.xpath("./@href").extract_first()
                all_city_url.append("https://www.liepin.com" + this_url)
